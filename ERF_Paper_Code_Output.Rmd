...
train.rf <- prediction(rf_train[,1], y_train)
test.rf <- prediction(rf_test[,1], y_test)
...
stargazer::stargazer(logistic_model,type='html',report = "vc*stp",
    ci = TRUE)
```
...
legend(0.32, 0.25, c("Logistic Regression Train Data", "Logistic Regression Test Data", "Random Forest Train Data", "Random Forest Test Data"), 
...
# Open a pdf file
jpeg("ROC_LR_RF.jpg", width = 700, height = 350, res = 300)
...
```{r roccurve, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE}
# Plot the ROC curve
plot(perf_train, main = "Logistic Regression vs Random Forest AUC-ROC Curve",
    col = "#009ECE")
plot(perf_test, add = T, col = "#FF9E00", lwd = 1.5)
